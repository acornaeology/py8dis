...
def label(runtime_addr, name, move_id=None, *,
          description=None, length=None, group=None, access=None):
    """Define a label name for a runtime address.

    Optional memory-map metadata for out-of-ROM labels (RAM workspace,
    RAM buffers, MMIO) that downstream structured consumers render as
    a dedicated memory-map page, and that the asm output appends as a
    trailing `;` brief on the equate line:

        label(0x0080, "mem_ptr_lo",
              description="Low byte of the indirect pointer used by "
                          "`ram_test` and the transmit/receive byte loops.",
              length=1, group="zero_page", access="rw")

    - `description`: Markdown. The first paragraph (up to the first
      blank line) is treated as the "brief" and appears both in the
      asm equate comment and on the memory-map page; subsequent
      paragraphs are extended detail shown only on the page.
    - `length`: number of bytes covered by this label (default 1 --
      used by the memory-map page to render `&025A-&0359` for a
      256-byte buffer).
    - `group`: free-form grouping key (e.g. `"zero_page"`,
      `"ram_buffers"`, `"io_a"`). The memory-map page groups entries
      by this value, preserving their declaration order within each
      group.
    - `access`: one of `"r"`, `"w"`, `"rw"`; shown on the memory-map
      page and useful for distinguishing read-only jumper reads from
      read-write workspace and write-only MMIO.

    All four kwargs are optional; the metadata is ignored for labels
    whose runtime address is inside the loaded ROM range.
    """
    ...
    label_obj = disassembly.add_label(runtime_addr, name, move_id, is_autogenerated=False)

    # Attach memory-map metadata if any was supplied. If the same
    # label is re-declared with new metadata, the new values win;
    # callers aren't expected to mix (it'd be confusing).
    if description is not None:
        label_obj.description = description
    if length is not None:
        label_obj.length = length
    if group is not None:
        label_obj.group = group
    if access is not None:
        label_obj.access = access
...
def subroutine(runtime_addr, name=None, title=None, description=None, on_entry=None, on_exit=None, hook=False, move_id=None, is_entry_point=True, at_binary_addr=False):
    """
    Define a subroutine.
    ...
    If at_binary_addr is True, the label and subroutine banner are
    placed at the binary/ROM address (before the org directive for
    any move block) rather than at the runtime address. This is
    useful for labelling relocated code blocks at their ROM position
    without creating duplicate symbol conflicts.
    """
    if at_binary_addr:
        # Place label and comments at the binary address with BASE_MOVE_ID.
        # The address passed in IS the binary address (not a runtime address
        # that needs resolving through a move).
        binary_addr = memorymanager.BinaryAddr(runtime_addr)
        base_move_id = movemanager.BASE_MOVE_ID
        rom_runtime_addr = memorymanager.RuntimeAddr(runtime_addr)
        binary_loc = memorymanager.BinaryLocation(binary_addr, base_move_id)

        # Resolve the actual runtime address for the move destination
        actual_move_id = movemanager.move_id_for_binary_addr[binary_addr]
        if actual_move_id != base_move_id:
            dest_runtime_addr = movemanager.b2r(binary_addr)
        else:
            dest_runtime_addr = rom_runtime_addr

        if name is not None and len(name) > 0:
            disassembly.add_label(rom_runtime_addr, name, base_move_id, is_autogenerated=False)

        if is_entry_point:
            # Register an entry point so the code gets traced.
            # Use a with block for the move so entry() can resolve the
            # runtime address within the move.
            if actual_move_id != base_move_id:
                with actual_move_id:
                    entry(dest_runtime_addr)
            else:
                entry(runtime_addr)
        ...
        if memorymanager.is_data_loaded_at_binary_addr(binary_addr):
            _emit_subroutine_banner(binary_loc, title, description, on_entry, on_exit)

        # Store with runtime_addr=destination so JSON addr matches items,
        # and binary_addr=ROM address for cross-referencing.
        trace.add_subroutine(dest_runtime_addr, name, title, description, on_entry, on_exit, hook, move_id, binary_addr=int(binary_addr))
        return
...
def _format_subroutine_middle(title, description, on_entry, on_exit):
    """Format the body text for a subroutine banner comment."""
    middle = ""
...
def _emit_subroutine_banner_runtime(runtime_addr, title, description, on_entry, on_exit, move_id):
    """Emit subroutine banner comments via auto_comment (runtime address path)."""
    ...
    middle = _format_subroutine_middle(title, description, on_entry, on_exit)
    if len(middle)>0:
...
def _emit_subroutine_banner(binary_loc, title, description, on_entry, on_exit):
    """Emit subroutine banner comments directly at a binary location."""
    if config.get_subroutine_header() is not None:
        disassembly.comment_binary(binary_loc, config.get_subroutine_header(), word_wrap=False, auto_generated=True)
    middle = _format_subroutine_middle(title, description, on_entry, on_exit)
    if len(middle)>0:
        disassembly.comment_binary(binary_loc, middle, word_wrap=True, auto_generated=True)
        if config.get_subroutine_footer() is not None:
            disassembly.comment_binary(binary_loc, config.get_subroutine_footer(), word_wrap=False, auto_generated=True)
